...
trait Stream[+A] {
  ...
  def zipWithAll[B, C](s2: Stream[B])(f: (Option[A], Option[B]) => C): Stream[C] =
    unfold((this, s2)) {
      case (Cons(x,xs), Cons(y,ys)) =>
        Some(f(Some(x()), Some(y())), (xs(), ys()))
      case (Cons(x,xs), empty) =>
        Some(f(Some(x()), Option.empty[B]), (xs(), empty))
      case (empty, Cons(y,ys)) =>
        Some(f(Option.empty[A], Some(y())), (empty, ys()))
      case _ => None
    }
...
object Stream {
  ...
  def fibs: Stream[Int] = {
    def go(pp: Int, p: Int): Stream[Int] =
      cons(pp, go(pp, pp+p))
    go(0, 1)
  }
  ...
  def fibsViaUnfold: Stream[Int] =
    unfold((0,1)) { case (pp, p) => Some(pp, (p, pp+p)) }
